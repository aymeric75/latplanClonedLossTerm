...
def thedef(domainfile, problem_dir, heuristics, cycle, sigma, sae=None, bar=None, epoch=None):
    ...
    bar.update(epoch+1, status = "[in planner 1] "+"\n")
    ...
    bar.update(epoch+1, status = "[in planner 2] "+"\n")
    ...
    try:
        ...
        bar.update(epoch+1, status = "[in planner 3] "+"\n")
        ...
        # INSTEAD of FD-lastest we call a lightweight planner (pyperplan)
        # fast-downward.py domain.pddl problem.pddl --search
        
        #echodo(["downward/fast-downward.py", problemfile, domainfile, options[heuristics]])

        echodo(["pyperplan", domainfile, problemfile])

        log(f"finished planning")


        bar.update(epoch+1, status = "[in planner 4] "+"\n")
        ...
        bar.update(epoch+1, status = "[in planner 5] "+"\n")
        ...
        bar.update(epoch+1, status = "[in planner 6] "+"\n")
        ...
        valid_bis = bool(np.all(validation_bis))

        bar.update(epoch+1, status = "[in planner 7] "+"\n")

        f = open("LossFile.txt", "w")
        if valid:
            log(f"transitions valid")
            f.write("1\n")
        else:
            f.write("0\n")
        f.close()
        ...
        bar.update(epoch+1, status = "[in planner 8] "+"\n")
