            ...
            for epoch in range(start_epoch,start_epoch+epoch):
                ...
                for train_subdata_cache,train_subdata_to_cache in zip(train_data_cache,train_data_to_cache):
                    for net,train_subdata_batch_cache,train_subdata_to_batch_cache in zip(self.nets, train_subdata_cache,train_subdata_to_cache):
                        ...
                        # test is_correct every 10 epochs
                        ...
                        # once is_correct is set to one, then test if able to construct a plan (at every batch!)
                        # if is_correct == 1:
                        ...
                        #     import subprocess
                        #     # produce DOMAIN.PDDL
                        #     cmd = ['/workspace/latplan/latplanClonedLossTerm/small-launch.sh']
                        ...
                        if is_correct == 0:
                            ...
                            problem_dir="problem-generators/vanilla/sokoban-2-False/003-000"
                            network_dir="samples/sokoban_sokoban_image-20000-global-global-2-train_20000_CubeSpaceAE_AMA4Conv_kltune2"
                            setup_planner_utils(self, problem_dir, network_dir, "ama3")
                            
                            p = puzzle_module(self)
                            self.parameters["generator"] = "latplan.puzzles.sokoban"
                            pp = "samples/sokoban_sokoban_image-20000-global-global-2-train_20000_CubeSpaceAE_AMA4Conv_kltune2/logs/05-09T16:42:26.372"
                            ...
                            domainfile = pp+"/"+"domain.pddl"
                            
                            os.path.exists(ig) or np.savetxt(ig,[bits],"%d")
                            self.echodo(["helper/ama3-problem.sh","problem.ig","problem.pddl"])
                            ...
                            self.echodo(["helper/fd-latest.sh", options["blind"], problemfile, domainfile])
                            print("finished planning")

                            # if it finds in generated output
                        ...
                        ########################################################################
                        ########################################################################
                        # compute the new loss term, write it in a file
                        # 
                        #
                        # 0) retrieve the value of x0y0
                        # if epoch > 2:
                        #     for i, (subdata, subdata_to) in enumerate(zip(val_data, val_data_to)):
                        #         allVals = net.evaluate(val_data,
                        #                             val_data_to,
                        #                             batch_size=batch_size,
                        #                             verbose=0)
                        #         print("x0y00 ")
                        #         print(allVals[13])
                        #         self.bar.update(epoch+1, status = "[vv] "+"  "+str(allVals[13]) + "\n")
                        # 1) if < 500, compute the loss term

                        # 2) test if reconstr is visually correct

                        # if epoch % 10 == 0 and epoch != 0 and some_count == 0:
                        #     # 
                        #     transitions, states = self.load_sokoban("sokoban_image-20000-global-global-2-train", 20000, False)
                        #     from latplan.util.planner import puzzle_module
                        #     self.parameters["generator"] = "latplan.puzzles.sokoban"
                        #     p = puzzle_module(self)
                        ...
                        #     is_correct = p.validate_statesBIS(np.array([image_rec]))
                        #     print("ici2")
                        #     self.bar.update(epoch+1, status = "[vvvv] "+"  "+str(is_correct) + " some_count " +str(some_count)+"\n")
                        #     self.plot_image([image_rec],w=10,path="reco_image"+str(epoch)+".png",verbose=False)
                        #     some_count+=1
                        #     #from latplan.main.puzzle import 
                        #     #self.load_puzzle("sokoban_image-20000-global-global-2-train", "", "", 20000)  
                        #     #exit()
                        ########################################################################
                        ########################################################################
