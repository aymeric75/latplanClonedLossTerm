...
class PreconditionMixin:
    ...
    def dump_preconditions(self, pre, suc, data, actions, histogram, true_num_actions, all_labels, action_ids, **kwargs):
        ...
        pos_precondition = np.concatenate((pos_precondition, neg_precondition), axis=1)
        neg_precondition = np.zeros(pos_precondition.shape)
...
# class Layer_forward_loss1(Layer):

#     def __init__(self, beta_z, kl_z0, kl_a_z0, beta_d, kl_z1z2):
#         self.beta_z = beta_z
#         self.kl_z0 = kl_z0
#         self.kl_a_z0 = kl_a_z0
#         self.beta_d = beta_d
#         self.kl_z1z2 = kl_z1z2
#         super().__init__()

#     def call(self, inputs):

#         retour = self.beta_z * self.kl_z0 + inputs[0] + self.kl_a_z0 + self.beta_d * self.kl_z1z2 + inputs[1]
#         return retour


class Layer_loss(Layer):

    def __init__(self, beta_z, kl_z0, kl_a_z0, beta_d, kl_z1z2, kl_z1, kl_a_z1, kl_z0z3):
        self.beta_z = beta_z
        self.kl_z0 = kl_z0
        self.kl_a_z0 = kl_a_z0
        self.beta_d = beta_d
        ...
        self.kl_z1 = kl_z1
        self.kl_a_z1 = kl_a_z1
        self.kl_z0z3 = kl_z0z3
    ...
    def call(self, inputs):
        ...
        ama3_forward_loss1  = self.beta_z * self.kl_z0 + inputs[0] + self.kl_a_z0 + self.beta_d * self.kl_z1z2 + inputs[1]
        ama3_forward_loss2  = self.beta_z * self.kl_z0 + inputs[0] + self.kl_a_z0 + inputs[2]
        ama3_backward_loss1 = self.beta_z * self.kl_z1 + inputs[1] + self.kl_a_z1 + self.beta_d * self.kl_z0z3 + inputs[0]
        ama3_backward_loss2 = self.beta_z * self.kl_z1 + inputs[1] + self.kl_a_z1 + inputs[3]


        return ama3_forward_loss1 + ama3_forward_loss2 + ama3_backward_loss1 + ama3_backward_loss2




class MyRegularizerLayer(Layer):
...
class BaseActionMixinAMA4Plus(BidirectionalMixin, BaseActionMixin):
    ...
    def add_(self, tensors):
        #return K.add(tensors[0],tensors[1])
        return tf.add(tf.add(tensors[0], tensors[1]), tf.add(tensors[2], tensors[3]))

    def totals_(self, tensors):
        return tf.abs(tf.reduce_mean(tensors))


    def _build_primary(self,input_shape):


        input = Input(shape=(2,*input_shape))

        #x = input
        x = MyRegularizerLayer()(input)
 
        #total_new_loss = Lambda(lambda x: tf.abs(tf.reduce_mean(x)))(x)
        ...
        # print("y aae") Tensor("dmerge_5/concat:0", shape=(?, 2, 28, 28, 3), dtype=float32)
        print(y_aae)
        ...
        # some tests ignore it!!
        # loss_trans = 0.
        # loss_states = 0.
        additional_loss = 0.
        # loss_trans = 0. if self.parameters["newLoss0"] == 999 else (-1 if self.parameters["newLoss0"] == 1 else 0.5)
        # loss_states = 0. if self.parameters["newLoss1"] == 999 else (-0.5 if self.parameters["newLoss1"] == 1 else 1)
        # print("NEWLOSS")
        # print(type(self.parameters["newLoss1"]))
        #additional_loss = self.parameters["newLoss"]
        #additional_loss = loss_trans + loss_states
        ...
        #!!Some tests, ignore it!!
        #total_loss = Lambda(self.add_)([ama3_forward_loss1, ama3_forward_loss2, ama3_backward_loss1, ama3_backward_loss2])
        #total_loss = Lambda(self.totals_)([x0y0, x1y1, x1y2, x0y3])    
        #total_loss = Lambda(lambda a: a[0] + a[1])([x, b])
        #total_loss = Add()([ama3_forward_loss1, ama3_forward_loss2, ama3_backward_loss1, ama3_backward_loss2])
        #total_loss = Layer_loss(self.parameters["beta_z"], kl_z0, kl_a_z0, self.parameters["beta_d"], kl_z1z2, kl_z1, kl_a_z1, kl_z0z3)([x0y0, x1y1, x1y2, x0y3])

        total_loss = (ama3_forward_loss1 + ama3_forward_loss2 + ama3_backward_loss1 + ama3_backward_loss2)/4 

        #!!Some tests, ignore it!!
        # one_tenth = total_loss / 10.
        # new_term_loss = one_tenth*additional_loss
        #total_loss += new_term_loss
        ...
        self.add_metric("x1y2",x1y2)
        self.add_metric("elbo",elbo)
        additional_loss = tf.convert_to_tensor(additional_loss, dtype=tf.float32)
        
        self.add_metric("additional_loss",additional_loss)
        #self.add_metric("one_tenth",one_tenth)
        def loss(*args):
            return total_loss
        ...
        # note: original z does not work because Model.save only saves the weights that are included in the computation graph between input and output.
        #self.net = Model(x, [y_aae, total_new_loss])
        #self.net = Model(x, y_aae)
        self.net = Model(input, [x, y_aae])

    
        #self.net.add_loss(tf.abs(tf.reduce_mean(x)))
        self.encoder     = Model(input, z) # note : directly through the encoder, not AAE
        self.autoencoder = Model(input, y) # note : directly through the decoder, not AAE
        ...
        # for plotting
        self._encode_prob     = Model(input, p) # note : directly through the encoder, not AAE
        self._encode_prob_aae = Model(input, p_aae) # note : directly through the encoder, not AAE
    ...
    # def evaluateX0Y0(self,*args,**kwargs):
    #     metrics = { k:v for k,v in zip(['x0y0'],
    #                                    ensure_list(self.net.evaluate(*args,**kwargs)))}
    #     return metrics["x0y0"]
